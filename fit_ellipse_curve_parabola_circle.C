...
void generate_pol1(vector<Double_t> &vec_x_fit, vector<Double_t> &vec_y_fit, Double_t pol1_k, Double_t pol1_b, Double_t xMin, Double_t xMax);
//
...
Bool_t fit_pol1(Double_t x0, Double_t y0, Double_t x2, Double_t y2, Double_t &pol1_k, Double_t &pol1_b);
...
Int_t fit_ellipse_curve_parabola_circle(){
  ...
  loadData(dataFileIn, grtmp);
  grtmp->GetPoint(0,xL,yL);
  grtmp->GetPoint((grtmp->GetN()-1),xR,yR);
  grtmp->Fit("pol8","","",xL,xR);
  TF1 *f_tmp_fit = (TF1*)grtmp->GetListOfFunctions()->FindObject("pol8");
  //cout<<"f_tmp_fit->GetParameter(0) = "<<f_tmp_fit->GetParameter(0)<<endl;
  //
  //fit
  ...
  //
  Double_t dx = 300.0/100.0;
  //01
  fitCurveTypeName = "pol1";
  x2 = 300;
  x0 = x2 - dx;
  y0 = f_fit->Eval(x0);
...
Bool_t fit( Double_t x0, Double_t y0, Double_t y0Der,
  ...
  Double_t pol1_k = -999.0;
  Double_t pol1_b = -999.0;
  ...
  else if(fitCurveTypeName == "pol1"){
    fit_result = fit_pol1( x0, y0, x2, y2, pol1_k, pol1_b);
    if(fit_result){
      generate_pol1(fit_state->vec_x_fit, fit_state->vec_y_fit, pol1_k, pol1_b, x0, x2);
      Double_t ytmp;
      Double_t yDertmp;
      fit_state->fit_x0 = x0;
      fit_state->fit_x2 = x2;
